...
module TwGlCCC {ℓ} (𝒞 : Contextual ℓ ℓ) ⦃ 𝒞CCC : CCC 𝒞 ⦄ (base : Char → Contextual.ty 𝒞) where
  ...
  open Enveloping 𝑃𝑆ℎ
  open Cartesian (PShCart {𝒞 = R.ambCat})
  open NatTrans
  open Functor

  A-A⇒B : (A B : Glueing) → C.ty
  ...
  private
    ...
    comp-A⇒B-ob A B i Γ M =
      ...
      Λ (𝑎𝑝𝑝 (ιNe (M [ W₁𝑅𝑒𝑛 𝐴 (id𝑅𝑒𝑛 Γ) ]NE)) C.𝑧)
        ≡⟨ (λ j → Λ (𝑎𝑝𝑝 (N-hom (ιNE (𝐴 ⇛ 𝐵)) (W₁𝑅𝑒𝑛 𝐴 (id𝑅𝑒𝑛 Γ)) j M) C.𝑧)) ⟩
      Λ (𝑎𝑝𝑝 (ιNe M C.⟦ C.makeRen (W₁𝑅𝑒𝑛 𝐴 (id𝑅𝑒𝑛 Γ)) ⟧) C.𝑧)
        ≡⟨ (λ j → Λ (𝑎𝑝𝑝 (ιNe M C.⟦ C.makeW₁ (id𝑅𝑒𝑛 Γ) j ⟧) C.𝑧)) ⟩
      Λ (𝑎𝑝𝑝 (ιNe M C.⟦ C.makeRen (id𝑅𝑒𝑛 Γ) C.⊚ C.π ⟧) C.𝑧)
        ≡⟨ (λ j → Λ (𝑎𝑝𝑝 (ιNe M C.⟦ C.𝒾𝒹η₂ j C.⊚ C.π ⟧) C.𝑧)) ⟩
      Λ (𝑎𝑝𝑝 (ιNe M C.⟦ C.𝒾𝒹 Γ C.⊚ C.π ⟧) C.𝑧)
        ≡⟨ (λ j → Λ (𝑎𝑝𝑝 (ιNe M C.⟦ C.𝒾𝒹L C.π j ⟧) C.𝑧)) ⟩
      Λ (𝑎𝑝𝑝 (ιNe M C.⟦ C.π ⟧) C.𝑧)
        ≡⟨ 𝑎𝑝𝑝η (ιNe M) ⁻¹ ⟩
      ιNe M
        ∎) i where
        𝐴 = Gl-A A
  ...
  N-ob (Gl-u (A ⇒TwGl B)) Γ M = N-ob (u-A⇒B A B) Γ M ,
    λ Δ σ x →
      ...
      𝑎𝑝𝑝 (ιNe M C.⟦ C.makeRen σ ⟧) (ιNf (N-ob (Gl-q A) Δ x))
        ≡⟨ (λ i → 𝑎𝑝𝑝 (N-ob (comp-A⇒B A B (~ i)) Γ M C.⟦ C.makeRen σ ⟧)
          (ιNf (N-ob (Gl-q A) Δ x))) ⟩
      𝑎𝑝𝑝 (ιNf (N-ob (q-A⇒B A B) Γ (N-ob (u-A⇒B A B) Γ M)) C.⟦ C.makeRen σ ⟧)
        (ιNf (N-ob (Gl-q A) Δ x))
  ...
  ΛTwGl-nat-ob : {Γ : Glueings} {A B : Glueing} → (t : GlTm (Γ ⊹ A) B) →
    ...
    ≡ N-ob (TMよ (Λ (GlTm-α t)) ⟦ ιNFS (Gls-Γ Γ) ⊚ Gls-q Γ ⟧)
  ΛTwGl-nat-ob {Γ} {A} {B} t i Δ 𝓈 =
    {!F-hom (⇓EnvCtx (Gls-⦇Γ⦈ Γ)) (W₁𝑅𝑒𝑛 𝐴 (id𝑅𝑒𝑛 Δ)) 𝓈
    --N-ob (ιNF 𝐵 𝒩∘ Gl-q B 𝒩∘ GlTm-⦇α⦈ t) (Δ ⊹ 𝐴)
    {-Λ (N-ob (ιNF 𝐵 𝒩∘ Gl-q B 𝒩∘ GlTm-⦇α⦈ t) (Δ ⊹ 𝐴)
      (F-hom (⇓EnvCtx (Gls-⦇Γ⦈ Γ)) (W₁𝑅𝑒𝑛 𝐴 (id𝑅𝑒𝑛 Δ)) 𝓈 , N-ob (Gl-u A) (Δ ⊹ 𝐴) (VN 𝑧𝑣)))-}
      

    {-(Λ (ιNf (N-ob (Gl-q B) (Δ ⊹ 𝐴) (N-ob (GlTm-⦇α⦈ t) (Δ ⊹ Gl-A A)
      (F-hom (⇓PSh (Gls-⦇Γ⦈ Γ)) (W₁𝑅𝑒𝑛 𝐴 (id𝑅𝑒𝑛 Δ)) 𝓈 ,
        N-ob (Gl-u A) (Δ ⊹ 𝐴) (VN 𝑧𝑣)))))
      ≡⟨ (λ j → Λ (N-ob (GlTm-nat t j) (Δ ⊹ 𝐴)
        (F-hom (⇓PSh (Gls-⦇Γ⦈ Γ)) (W₁𝑅𝑒𝑛 𝐴 (id𝑅𝑒𝑛 Δ)) 𝓈 ,
          N-ob (Gl-u A) (Δ ⊹ 𝐴) (VN 𝑧𝑣)))) ⟩
    Λ (N-ob (TMよ (GlTm-α t) ⟦ ιNFS (Gls-Γ (Γ ⊹ A)) ⊚ Gls-q (Γ ⊹ A) ⟧)
