...
module _ where
    ...
    Var-nat-ob {Γ ⊹ B} {A} (𝑠𝑣 v) i Δ (MS , M) =
      ...
      C.derive (varify (W₁Ren (Gl-A B) (idRen (mapRL Gl-A Γ)))) (TmVar v)
        [ ⇓TMS (N-ob (⇓PShMor (ιNFS (Gls-Γ Γ) ⊚ Gls-q Γ ⊚ π)) Δ (MS , M))
          ⊕ ιNf (Gl-q B .N-ob Δ M) ]
        ≡⟨ (λ i →
          N-ob (TMよ (C.makeVar (𝑠𝑣 (TmVar v)))
            ⟦ ×tmLem2 (ιNFS (Gls-Γ Γ)) (ιNF (Gl-A B)) (Gls-q Γ) (Gl-q B) (~ i) ⟧) Δ (MS , M)) ⟩
      N-ob (TMよ (C.makeVar (𝑠𝑣 (TmVar v))) ⟦ ιNFS (Gls-Γ (Γ ⊹ B)) ⊚ Gls-q (Γ ⊹ B) ⟧)
        Δ (MS , M)
        ∎) i
  ...
  W₁TwGlVarLem : {Γ Δ : Glueings} {A B : Glueing} (v : ExtVar Δ B) (τ : GlTms Γ Δ)
    (t : GlTm Γ A) → makeTwGlVar (𝑠𝑣 v) [ τ ⊕ t ]Gl ≡ makeTwGlVar v [ τ ]Gl
  GlTm-⦇α⦈ (W₁TwGlVarLem v τ t i) = W₁⟦⟧ (PShVar v) (GlTms-⦇αs⦈ τ) (GlTm-⦇α⦈ t) i
  GlTm-α (W₁TwGlVarLem v τ t i) = C.W₁⟦⟧ (TmVar v) (GlTms-αs τ) (GlTm-α t) i
  GlTm-nat (W₁TwGlVarLem {Γ} {Δ} {A} {B} v τ t i) j =
    isSet→SquareP (λ i j → isSetNat)
      (GlTm-nat (makeTwGlVar (𝑠𝑣 v) [ τ ⊕ t ]Gl))
      (GlTm-nat (makeTwGlVar v [ τ ]Gl))
      (λ k → (ιNF (Gl-A B) 𝒩∘ Gl-q B) 𝒩∘ GlTm-⦇α⦈ (W₁TwGlVarLem v τ t k))
      (λ k → TMよ (GlTm-α (W₁TwGlVarLem v τ t k)) ⟦ ιNFS (Gls-Γ Γ) ⊚ Gls-q Γ ⟧) i j

  W₁TwGlRenLem : {Γ Δ Σ : Glueings} {A : Glueing} (σ : ExtRen Δ Σ) (τ : GlTms Γ Δ)
    (t : GlTm Γ A) → makeTwGlRen (W₁𝑅𝑒𝑛 σ) ∘GlTms (τ ⊕ t) ≡ makeTwGlRen σ ∘GlTms τ
  W₁TwGlRenLem ! τ t = refl
  W₁TwGlRenLem (σ ⊕ v) τ t i = W₁TwGlRenLem σ τ t i ⊕ W₁TwGlVarLem v τ t i

  idTwGlL : {Γ Δ : Glueings} (σ : GlTms Γ Δ) → idTwGl Δ ∘GlTms σ ≡ σ
  idTwGlL ! = refl
  idTwGlL {Γ} {Δ ⊹ A} (σ ⊕ t) i =
    ((makeTwGlRen (W₁𝑅𝑒𝑛 (id𝑅𝑒𝑛 Δ)) ∘GlTms (σ ⊕ t)) ⊕ (makeTwGlVar 𝑧𝑣 [ σ ⊕ t ]Gl)
      ≡⟨ (λ k → W₁TwGlRenLem (id𝑅𝑒𝑛 Δ) σ t k ⊕ 𝑧𝑣TWGlLem σ t k) ⟩
